...
from apc.outlet import (
    OutletsException,
    OutletStatusParseException,
    OutletParseException,
)
...
def test_outlet_contructor():
    ...
    assert (
        str(ol) == "Outlet 2 AAAAAAAAAAAAAAA OFF*"
    )  # display name with only 15 characters
...
def test_outlets_collection():
    with pytest.raises(OutletsException):
        ol_collection = Outlets(
            [Outlet(1, "MyServer#1", "OFF"), Outlet(1, "MyServer#1", "ON")]
        )
